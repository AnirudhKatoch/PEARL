...
def trial_function(pf_new,sim_dir):
...
# (-0.866, "Simulation_22"), (+0.866, "Simulation_23"),

def run_one(pf_new, name):
    sim_dir = str(Path("Results") / name)
    trial_function(pf_new, sim_dir)

if __name__ == "__main__":
    mp.set_start_method("spawn")
    for pf_new, name in schedule:
        p = mp.Process(target=run_one, args=(pf_new, name))
        p.start()
        p.join()
        if p.exitcode != 0:
