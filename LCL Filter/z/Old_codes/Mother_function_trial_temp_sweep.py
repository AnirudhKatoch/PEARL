import numpy as np
from Input_parameters_mini_grid import Input_parameters_class
...
def mother_temp_sweep(City,sim_dir):
    ...
    Profile_size = params.Profile_size; Vdc_RMS = params.Vdc_RMS; M = params.M; Vo = params.Vo; Vg_RMS = params.Vg_RMS #S_RMS = params.S_RMS;
    pf = params.pf; P_RMS = params.P_RMS; Q_RMS = params.Q_RMS; Ig_RMS = params.Ig_RMS
    #T_amb = params.T_amb;
    ...
    dataframes_dir = str(Path(sim_dir) / "Dataframes")
    figures_dir = str(Path(sim_dir) / "Figures")
    ...
    df_S_profile = pd.read_parquet("z/Load profiles/Load_profiles.parquet")
    S_RMS = df_S_profile["Power"].to_numpy()

    df_T_amb_profile = pd.read_parquet("z/Temperature cities/Temperature_2025_daily.parquet")
    T_amb = df_T_amb_profile[City].to_numpy() + 273.15
    ...
    Plotting_function.plot_lifetime_monte_carlo(Lifetime_C_MC, Lifetime_L1_MC, Lifetime_L2_MC,Lifetime_LCL_MC, figures_dir=figures_dir,plot_type="line")
    ...
    print(f"{City}  B10 [years]  C={B10_C:.2f}  L1={B10_L1:.2f}  L2={B10_L2:.2f}  LCL={B10_LCL:.2f}")



Cities = ['Munich', 'Koudougou', 'Bahir_Dar', 'Kumasi', 'Mumbai', 'Bandung',
          'Juja', 'Windhoek', 'Lima', 'Stellenbosch', 'Kampala']

def run_one(city):
    sim_dir = str(Path("Results") / city)
    mother_temp_sweep(city, sim_dir)

if __name__ == "__main__":
    mp.set_start_method("spawn")
    for city in Cities:
        p = mp.Process(target=run_one, args=(city,))
        p.start()
        p.join()
        if p.exitcode != 0:
            raise RuntimeError(f"{city} failed with exit code {p.exitcode}")