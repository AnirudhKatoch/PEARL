...
class Input_parameters_class:
    ...
    def __init__(self):
        ...
        self.Vdc_rated = 700                              # [V] Rated DC bus voltage, defines maximum available inverter voltage level
        self.Vo_rated = 350                               # [V] Rated PWM pulse amplitude, instantaneous switched level (±Vo), must be ≤ allowed by topology
        ...
        self.S_RMS = np.full(self.Profile_size,50e3)                          # [VA] Mission profile of apparent power sampled at 1-second intervals
        ...
        self.Vg_ll_RMS = 400                                            # [V] RMS of fundamental line-to-line grid voltage
        self.S_rated = 50e3                                             # [VA] Rated apparent Inverter  power
        ...
        self.L1_specs = {
            ...
            'A_core': 180e-3 * 0.559,    # [m]  Overall width;         outer horizontal dimension;                    from [A] Table 1
            'B_core': 240e-3 * 0.559,    # [m]  Overall height;        outer vertical dimension;                      from [A] Table 1
            'D_core': 30e-3  * 0.559,    # [m]  Depth (cast width);    dimension going into the page;                 from [A] Table 1
            'E_core': 50e-3  * 0.559,    # [m]  Thickness (build);                                                    from [A] Table 1
            'F_core': 80e-3  * 0.559,    # [m]  Window width;          inner horizontal opening for winding;          from [A] Table 1
            'G_core': 140e-3 * 0.559,    # [m]  Window height;         inner vertical opening for winding;            from [A] Table 1
        ...
        self.L2_specs = {
            ...
            # ── Core geometry (scaled 4216L1R-B × 1.55) ──────────────────────────
            'A_core': 180e-3 * 0.245,  # [m]  Overall width;         outer horizontal dimension;                    from [A] Table 1
            'B_core': 240e-3 * 0.245,  # [m]  Overall height;        outer vertical dimension;                      from [A] Table 1
            'D_core': 30e-3  * 0.245,  # [m]  Depth (cast width);    dimension going into the page;                 from [A] Table 1
            'E_core': 50e-3  * 0.245,  # [m]  Thickness (build);                                                    from [A] Table 1
            'F_core': 80e-3  * 0.245,  # [m]  Window width;          inner horizontal opening for winding;          from [A] Table 1
            'G_core': 140e-3 * 0.245,  # [m]  Window height;         inner vertical opening for winding;            from [A] Table 1
            'kf'    : 0.82,            # [-]  Stacking factor;                                                      from [A] Table 2
        ...
        self.C_specs = {
            ...
            # Product code - B32361A4306J080  # FilterCap MKD AC – Single phase # TDK Electronics

            # Capacitance
            'C': 30e-6,  # [F]    Capacitance

            # Dimensions (Can be both cylinder or box shape)
            'D_case': None,
            'L_case': 57.5e-3,
            'H_case': 50e-3,
            'W_case': 40e-3,
            ...
            # Ratings
            'I_C_RMS_rated': 14,              # [A]     Rated RMS current
            'V_C_RMS_Rated': 275,             # [V]     Rated RMS voltage
            'V_C_Peak_Rated': 275*np.sqrt(2),            # [V]     Rated peak voltage
            'Temperature_Rated': 273.15 + 60, # [K]     Rated temperature
            ...
            # Loss model
            'Rs': 7.5e-3,  # [Ohm]  ESR — series resistance of capacitor itself
            'tan_delta_measured' : 1.5e-3, # [-]
            'f_measured_for_tan_delta':1000,  # [Hz]
            'tan_delta_0': None,
            ...
            # Damping resistor
            'R3': 0.06925 # [Ohm]  Series resistance placed in parallel with capacitor for passive damping of LCL resonance (PD-3 method)
        }
