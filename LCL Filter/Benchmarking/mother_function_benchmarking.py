...
Vg_RMS = Calculation_functions.Singal_RMS(Signal=Vg, resolution_per_cycle=resolution_per_cycle, f=f)
Vs_RMS = Calculation_functions.Singal_RMS(Signal=Vs, resolution_per_cycle=resolution_per_cycle, f=f)


# ----------------------------------------#
# all the benchmarking stuff
# ----------------------------------------#
Power_sweep = int((S_RMS[0]/S_rated)*100)
pf_sweep = pf[0]
if pf_sweep<0:
    pf_sweep = f"{pf_sweep*-1}_lags"
elif pf_sweep>0 and pf_sweep<1:
    pf_sweep = f"{pf_sweep}_leads"
elif pf_sweep==0 or pf_sweep==1:
    pf_sweep = f"{pf_sweep}"


def print_benchmark_values(V_L1_RMS, I_L1_RMS, V_L2_RMS, I_L2_RMS, I_C_RMS,V_C_RMS, Vg_RMS, Vs_RMS, THD_percent_I_L2,I_L2, Vg, f, dt, n_cycles=1,label=None):
    ...
    # --- grid-side power factor: fundamental phase of I_L2 relative to Vg ---
    I_seg_full = np.asarray(I_L2, dtype=float).ravel()
    Vg_full = np.asarray(Vg, dtype=float).ravel()
    samples_per_cycle = int(round(1.0 / (f * dt)))
    N = n_cycles * samples_per_cycle
    if I_seg_full.size < N or Vg_full.size < N:
        raise ValueError(f"Signals too short: need {N} samples, "
                         f"got I_L2={I_seg_full.size}, Vg={Vg_full.size}.")
    t = np.arange(N) * dt
    ref = np.exp(-1j * 2 * np.pi * f * t)
    phase_I = np.angle(np.sum(I_seg_full[-N:] * ref))
    phase_V = np.angle(np.sum(Vg_full[-N:] * ref))
    delta = np.angle(np.exp(1j * (phase_V - phase_I)))   # wrapped to (-pi, pi]
    delta_deg = np.degrees(delta)
    pf = float(np.cos(delta))
    ...
    rows = [
        ("V_L1_RMS",          last(V_L1_RMS),         "V"),
        ("I_L1_RMS",          last(I_L1_RMS),         "A"),
        ("V_L2_RMS",          last(V_L2_RMS),         "V"),
        ("I_L2_RMS",          last(I_L2_RMS),         "A"),
        ("I_C_RMS",           last(I_C_RMS),          "A"),
        ("V_C_RMS",           last(V_C_RMS),          "V"),
        ("Vg_RMS",            last(Vg_RMS),           "V"),
        ("Vs_RMS",            last(Vs_RMS),           "V"),
        ("THD_percent_I_L2",  last(THD_percent_I_L2), "%"),
    ]
    ...
    print("=" * line)
    print("Benchmark values" + (f"  [{label}]" if label else ""))
    print("-" * line)
    for name, value, unit in rows:
        print(f"{name:<{width}} = {value:>14.4f} {unit}")
    print("-" * line)
    print(f"{'Power factor (I_L2 vs Vg)':<{width}} = {pf:>14.4f}  ({sense})")
    print(f"{'Phase diff (Vg - I_L2)':<{width}} = {delta_deg:>14.3f} deg")
    print("=" * line)

    return pf, delta_deg, sense
pf, delta_deg, sense =  print_benchmark_values(V_L1_RMS, I_L1_RMS, V_L2_RMS, I_L2_RMS, I_C_RMS,V_C_RMS, Vg_RMS, Vs_RMS, THD_percent_I_L2,I_L2=I_L2, Vg=Vg, f=f, dt=dt,label=f"{Power_sweep}_{pf_sweep}")


# For Benchmarking
def save_plecs_csv(name, signal,resolution_per_cycle,dt):
        """Write timestamp,value rows (PLECS From File format), first cycle only."""
        benchmark_dir = "Benchmarking/CSV_inputs"
        os.makedirs(benchmark_dir, exist_ok=True)
        one_cycle = resolution_per_cycle*10
        t_cycle = np.arange(one_cycle) * dt
        data = np.column_stack([t_cycle, signal[:one_cycle]])

        np.savetxt(os.path.join(benchmark_dir, name), data,fmt="%.10e", delimiter=",")
save_plecs_csv(name=f"Vs_{Power_sweep}_{pf_sweep}.csv", signal=Vs, resolution_per_cycle=resolution_per_cycle, dt=dt)
save_plecs_csv(name=f"Vg_{Power_sweep}_{pf_sweep}.csv", signal=Vg, resolution_per_cycle=resolution_per_cycle, dt=dt)

def save_python_benchmark(Results_root, Power_sweep, pf_sweep,V_L1_RMS, I_L1_RMS, V_L2_RMS, I_L2_RMS, I_C_RMS,V_C_RMS, Vg_RMS, Vs_RMS, THD_percent_I_L2,pf, delta_deg, sense,I_L2, dt, f, n_cycles):
    ...
    run_folder = os.path.join(Results_root, f"{Power_sweep}_{pf_sweep}")
    os.makedirs(run_folder, exist_ok=True)

    values = {
        "Power_sweep": Power_sweep,
        "pf_sweep": pf_sweep,
        "V_L1_RMS": last(V_L1_RMS),
        "I_L1_RMS": last(I_L1_RMS),
        "V_L2_RMS": last(V_L2_RMS),
        "I_L2_RMS": last(I_L2_RMS),
        "I_C_RMS":  last(I_C_RMS),
        "V_C_RMS":  last(V_C_RMS),
        "Vg_RMS":   last(Vg_RMS),
        "Vs_RMS":   last(Vs_RMS),
        "THD_percent_I_L2": last(THD_percent_I_L2),
        "PF": float(pf),
        "phase_deg": float(delta_deg),
        "sense": sense,
    }

    txt_path = os.path.join(run_folder, "benchmark_values.txt")
    with open(txt_path, "w") as fh:
        fh.write(repr(values))

    I = np.asarray(I_L2, dtype=float).ravel()
    N = n_cycles * int(round(1.0 / (f * dt)))
    wave = np.column_stack([np.arange(N) * dt, I[-N:]])

    csv_path = os.path.join(run_folder, "IL2_waveform.csv")
    np.savetxt(csv_path, wave, fmt="%.10e", delimiter=",",
               header="time,I_L2", comments="")
    return run_folder
save_python_benchmark(Results_root=r"E:\PEARL\LCL Filter\Benchmarking\Results\Python",Power_sweep=Power_sweep, pf_sweep=pf_sweep,V_L1_RMS=V_L1_RMS, I_L1_RMS=I_L1_RMS, V_L2_RMS=V_L2_RMS, I_L2_RMS=I_L2_RMS,I_C_RMS=I_C_RMS, V_C_RMS=V_C_RMS, Vg_RMS=Vg_RMS, Vs_RMS=Vs_RMS,THD_percent_I_L2=THD_percent_I_L2,pf=pf, delta_deg=delta_deg, sense=sense,I_L2=I_L2, dt=dt, f=f, n_cycles=10)
