...
def simulate_igbt_diode_cauer(r_I, cap_I,
    ...
    sol = solve_ivp(rhs, (t_start, t_end), T0, t_eval=t_eval, method=method, rtol=rtol, atol=atol )
...
def simulate_igbt_diode_cauer_new(r_I, cap_I,
                              r_D, cap_D,
                              r_paste, cap_paste,
                              r_sink, cap_sink,
                              P_I, P_D,
    ...
    dTdt_work = np.zeros(total_nodes, dtype=float)
    ...
    def compute_constant_jacobian():
        T_ref = np.zeros(total_nodes, dtype=float)   # any reference T works for linear systems
        t_ref = 0.0
        ...
        for j in range(total_nodes):
            T_pert = T_ref.copy()
            T_pert[j] += eps
            f_pert = rhs(t_ref, T_pert)
            A[:, j] = (f_pert - f0) / eps

        return A

    A_const = compute_constant_jacobian()

    def jac(t, T):
        # Jacobian does not depend on t or T because system is linear in T
        return A_const


    sol = solve_ivp(rhs, (t_start, t_end), T0, t_eval=t_eval, method=method, rtol=rtol, atol=atol,jac=jac)
